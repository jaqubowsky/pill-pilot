...
export function PreviewBlock({
	...
	const sortableIds = supplements.map((s) => s._id);
	...
	return (
		<div className="bg-surface-raised border border-edge-subtle rounded-xl shadow-sm p-md flex flex-col gap-md">
			...
			<SortableContext id={timeBlock.id} items={sortableIds} strategy={verticalListSortingStrategy}>
				<div ref={setNodeRef} className="flex flex-col divide-y divide-edge-subtle min-h-[2rem]">
					{supplements.map((supplement) => {
						...
						const prerequisite = supplement.prerequisiteLocalId
							? allSupplements.find((s) => s._id === supplement.prerequisiteLocalId)
							: null;
						...
						return (
							<PreviewSupplementRow
								...
								prerequisiteName={prerequisite?.name ?? null}
