...
export function usePreviewSupplementSheet({
	...
	const methods = useForm<PreviewSupplementSheetValues>({
		...
		defaultValues: {
			...
			prerequisiteLocalId: supplement?.prerequisiteLocalId ?? undefined,
			delayDays: supplement?.delayDays ?? undefined,
	...
	function handleSubmit(values: PreviewSupplementSheetValues) {
		const editedSupplement: EditedSupplement = {
			...
			prerequisiteLocalId: values.prerequisiteLocalId ?? null,
			delayDays: values.delayDays ?? null,
			confidence: supplement?.confidence ?? 1,
			schedules: isNew
				? [
						{
