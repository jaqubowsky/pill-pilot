...
SELECT id INTO STRICT v_tb_snack     FROM time_blocks WHERE user_id = v_user_id AND sort_order = 2 AND active = true;
SELECT id INTO STRICT v_tb_pre_lunch FROM time_blocks WHERE user_id = v_user_id AND sort_order = 3 AND active = true;
SELECT id INTO STRICT v_tb_lunch     FROM time_blocks WHERE user_id = v_user_id AND sort_order = 4 AND active = true;
SELECT id INTO STRICT v_tb_pre_dinner FROM time_blocks WHERE user_id = v_user_id AND sort_order = 5 AND active = true;
SELECT id INTO STRICT v_tb_dinner    FROM time_blocks WHERE user_id = v_user_id AND sort_order = 6 AND active = true;
SELECT id INTO STRICT v_tb_post_dinner FROM time_blocks WHERE user_id = v_user_id AND sort_order = 7 AND active = true;
SELECT id INTO STRICT v_tb_bedtime   FROM time_blocks WHERE user_id = v_user_id AND sort_order = 8 AND active = true;
...
INSERT INTO protocol_supplements (id, protocol_id, supplement_id, notes, is_critical, sort_order, start_day_offset, active)
VALUES ('seed_ps_ibuprofen_01', 'seed_proto_edge_case01', 'seed_supp_ibupr_0001',
  'Co 8h, max 3 tabletki/dzień. NIE na pusty żołądek', true, 0, 0, true);

-- (B) Cycling 5/2 (Ashwagandha)
INSERT INTO protocol_supplements (id, protocol_id, supplement_id, notes, is_critical, cycle_days_on, cycle_days_off, sort_order, start_day_offset, active)
VALUES ('seed_ps_ashwagand_01', 'seed_proto_edge_case01', 'seed_supp_ashwa_0001',
  'Cyklicznie 5/2. Nie łączyć z kofeiną', false, 5, 2, 1, 0, true);

-- (C) Cycling 30/30 (Kreatyna)
INSERT INTO protocol_supplements (id, protocol_id, supplement_id, notes, is_critical, cycle_days_on, cycle_days_off, sort_order, start_day_offset, active)
VALUES ('seed_ps_kreatyna_01', 'seed_proto_edge_case01', 'seed_supp_kreatyna_01',
  'Cykl 30/30. Po treningu z węglowodanami', false, 30, 30, 2, 0, true);

-- (D) Delayed start: L-Teanina starts day 7
INSERT INTO protocol_supplements (id, protocol_id, supplement_id, notes, is_critical, sort_order, start_day_offset, active)
VALUES ('seed_ps_lteanina_01', 'seed_proto_edge_case01', 'seed_supp_teani_0001',
  'Rozpocząć od dnia 7. Stock tracking OFF', false, 3, 7, true);

-- (E) Scoop unit, low stock
INSERT INTO protocol_supplements (id, protocol_id, supplement_id, notes, is_critical, sort_order, start_day_offset, active)
VALUES ('seed_ps_kolagen_001', 'seed_proto_edge_case01', 'seed_supp_kolag_0001',
  'Rano na czczo, w ciepłej wodzie', false, 4, 0, true);

-- (F) Drops unit, 2 blocks
INSERT INTO protocol_supplements (id, protocol_id, supplement_id, notes, is_critical, sort_order, start_day_offset, active)
VALUES ('seed_ps_k2mk7_00001', 'seed_proto_edge_case01', 'seed_supp_k2mk7_0001',
  'Z tłustym posiłkiem. Synergicznie z D3', false, 5, 0, true);

-- (G) Sachet unit
INSERT INTO protocol_supplements (id, protocol_id, supplement_id, notes, is_critical, sort_order, start_day_offset, active)
VALUES ('seed_ps_probiotyk_1', 'seed_proto_edge_case01', 'seed_supp_probi_0001',
  'Na pusty żołądek, min 30 min przed jedzeniem', false, 6, 0, true);

-- (H) Spray unit, single block
INSERT INTO protocol_supplements (id, protocol_id, supplement_id, notes, is_critical, sort_order, start_day_offset, active)
VALUES ('seed_ps_melatoni_01', 'seed_proto_edge_case01', 'seed_supp_melat_0001',
  '30 min przed snem, pod język', false, 7, 0, true);

-- (I) Delayed start: NAC starts day 14 (after medications)
INSERT INTO protocol_supplements (id, protocol_id, supplement_id, notes, is_critical, sort_order, start_day_offset, active)
VALUES ('seed_ps_nac_edge_01', 'seed_proto_edge_case01', (SELECT id FROM supplements WHERE user_id = v_user_id AND name = 'NAC' AND active = true LIMIT 1),
  'Rozpocząć od dnia 14 (po lekach)', false, 8, 14, true);
...
VALUES ('seed_ps_d3_shared_1', 'seed_proto_edge_case01', (SELECT id FROM supplements WHERE user_id = v_user_id AND name = 'Witamina D3 4000 IU' AND active = true LIMIT 1),
  'Współdzielony z Protokołem H.Pylori — wspólny stock', false, 9, 0, true);
...
VALUES ('seed_ps_pepzin_lo_1', 'seed_proto_edge_case01', (SELECT id FROM supplements WHERE user_id = v_user_id AND name = 'Pepzin (karnozynian cynku)' AND active = true LIMIT 1),
  'Niski stock! Sprawdź alert. 30 min przed jedzeniem', false, 10, 0, true);

-- (L) Zero stock + cycling + critical (Metronidazol) — starts day 14
INSERT INTO protocol_supplements (id, protocol_id, supplement_id, notes, is_critical, cycle_days_on, cycle_days_off, sort_order, start_day_offset, active)
VALUES ('seed_ps_metro_zer_1', 'seed_proto_edge_case01', (SELECT id FROM supplements WHERE user_id = v_user_id AND name = 'Metronidazol 500mg' AND active = true LIMIT 1),
  'Stock = 0! Lek na receptę. Cykl 14/14', true, 14, 14, 11, 14, true);

-- (M) Gram unit (Elektrolity)
INSERT INTO protocol_supplements (id, protocol_id, supplement_id, notes, is_critical, sort_order, start_day_offset, active)
VALUES ('seed_ps_electr_001', 'seed_proto_edge_case01', 'seed_supp_electr_001',
  '5g w wodzie, 2x dziennie', false, 12, 0, true);

-- (N) ML unit, no price (MCT Oil)
INSERT INTO protocol_supplements (id, protocol_id, supplement_id, notes, is_critical, sort_order, start_day_offset, active)
VALUES ('seed_ps_mctoil_001', 'seed_proto_edge_case01', 'seed_supp_mctoil_001',
  'Do kawy rano. Brak ceny w systemie', false, 13, 0, true);

-- (O) Portion unit (Colostrum)
INSERT INTO protocol_supplements (id, protocol_id, supplement_id, notes, is_critical, sort_order, start_day_offset, active)
VALUES ('seed_ps_colost_001', 'seed_proto_edge_case01', 'seed_supp_colost_001',
  'Na czczo, 30 min przed posiłkiem', false, 14, 0, true);

-- (P) Emanera — "przed kolacją" block
INSERT INTO protocol_supplements (id, protocol_id, supplement_id, notes, is_critical, sort_order, start_day_offset, active)
VALUES ('seed_ps_emanera_001', 'seed_proto_edge_case01', 'seed_supp_emanera_01',
  '30 min PRZED kolacją', true, 15, 0, true);

-- (Q) S. Boulardii — "po kolacji" block
INSERT INTO protocol_supplements (id, protocol_id, supplement_id, notes, is_critical, sort_order, start_day_offset, active)
VALUES ('seed_ps_sbould_0001', 'seed_proto_edge_case01', 'seed_supp_sbould_001',
  'PO kolacji', false, 16, 0, true);
...
-- Emanera: 1x (Przed kolacją) — capsule
INSERT INTO supplement_schedules (id, protocol_supplement_id, time_block_id, dosage_amount, dosage_unit) VALUES
  ('seed_ss_ema_predin_01', 'seed_ps_emanera_001', v_tb_pre_dinner, 1, 'capsule');

-- S. Boulardii: 1x (Po kolacji) — capsule
INSERT INTO supplement_schedules (id, protocol_supplement_id, time_block_id, dosage_amount, dosage_unit) VALUES
  ('seed_ss_sbo_postdin_1', 'seed_ps_sbould_0001', v_tb_post_dinner, 1, 'capsule');
